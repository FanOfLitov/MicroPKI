...
def generate_crl(
    ...
    for record in revoked_certs_data:
        ...
        if reason_str:
            try:
                ...
                logger.warning("Unknown revocation reason '%s' for serial %s, omitting extension.", reason_str,
                               record["serial_hex"])
    ...
    logger.info("CRL generation complete. CA: %s, Number: %d, Revoked Count: %d, Path: %s", ca_name, crl_number, count,
                crl_path)
    return crl_path