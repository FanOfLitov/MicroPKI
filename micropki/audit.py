...
def verify_audit_log(log_file: str) -> bool:
        ...
        for idx, line in enumerate(f):
            try:
                ...
                if computed_hash_hex != recorded_hash:
                    logger.error("Audit log tampered at line %d! Expected: %s, Found: %s", idx + 1, computed_hash_hex,
                                 recorded_hash)
                    return False
    ...
    if stored_hash != current_hash:
        logger.error("Final hash does not match chain.dat! Expected %s, Found in chain %s", current_hash.hex(),
                     stored_hash.hex())
        return False
